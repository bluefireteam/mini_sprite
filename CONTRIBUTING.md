...
- Check that all tests pass: `melos test:select`;
...
- Clone the forked repository to your local development machine
   (e.g. `git clone git@github.com:<YOUR_GITHUB_USER>/mini_sprite.git`).


### Environment Setup

Mini Sprite is setup to run with the most recent `stable` version of Flutter, so make sure your
...
Also, Mini Sprite uses [Melos] to manage the project and dependencies.
...
The bootstrap command resolves all the packages in the [pub workspace] together, so that all
packages, examples and tests build from the local clone of the project. You should only need to run
this command once.
...
- When committing your changes, make sure that each commit message is clear
 (e.g. `git commit -m 'Add support for multiple colors to MiniSprite'`).
...
Go to the [pull request page of Mini Sprite][PRs] and in the top
...
mark (e.g. `feat!: Remove the pixels argument from MiniSprite`).
...
These instructions are for the maintainers of Mini Sprite.
...
- Run the `Prepare release` workflow from the Actions tab. It runs `melos version`, which bumps
   the versions and generates the `CHANGELOG.md` files, and opens a release PR.
...
- Once the release PR is merged, the `Tag release` workflow tags each released package and triggers
   the `Publish packages` workflow, which publishes the packages to pub.dev and creates the GitHub
   releases.


[GitHub issue]: https://github.com/bluefireteam/mini_sprite/issues
[GitHub issues]: https://github.com/bluefireteam/mini_sprite/issues
[PRs]: https://github.com/bluefireteam/mini_sprite/pulls
